...
3. **全额成交**（无部分成交）；委托数量向下取整到整股；佣金 `max(0.0005 * qty * price, 0)`，无滑点。订单当日有效：当日未成交的挂单由 `broker.cancel_unfilled()` 作废，不会滞留到之后日期按过时价格成交。
...
- `paper-live.py` 只在美股交易时段（Mon–Fri 09:30–16:00 ET，`engine/session.py`）扫描。信号用**已收盘日线**（~420 天 1d K 线，剔除当日未完成 bar），按**当日开盘价**记账——与回测"信号日收盘 → 次日开盘成交"一致；每交易日上午扫一次即可，不再 5m 盯盘。数据须过 `validate_daily`（bar 数 ≥260 / 非正价 / 单日 >25% 异动）与跨标的同价检测，不合格不出信号。paper 仓位与 ATR 止损按 `symbol|strategy` 隔离（`strategy/base.py` 的 `position_key`），旧 state 加载时自动迁移。
